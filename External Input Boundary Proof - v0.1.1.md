...
**Supersedes:** [External Input Boundary Proof — v0.1.0](External%20Input%20Boundary%20Proof%20-%20Draft.md) for the self-hash correction only. The original remains immutable and inspectable.
...
**Simulation version:** `0.7.0-draft.45` — fixed for this corrected proof.
...
`HF = canonical_hash(Rfinal)`, and `HC = canonical_hash(Rcontrol_final)` are
computed only after their complete successor records exist. No canonical record
or canonical ledger entry stores its own successor hash. These values are
verified externally against the fixed serializer.
...
Successor identity is verified only after each canonical record is complete:

```text
R0
  → LQ.canonical_pre_state_hash = H0
  → Rinput complete
  → HI = canonical_hash(Rinput)

Rinput
  → LAlphaFailed.canonical_pre_state_hash = HI
  → Rfinal complete
  → HF = canonical_hash(Rfinal)

R0 without Q
  → LAlphaSucceeded.canonical_pre_state_hash = H0
  → Rcontrol_final complete
  → HC = canonical_hash(Rcontrol_final)
```

The verifier and evidence must assert all of these external identities, plus
the matching successor `canonical_ancestry.parent_record_hash`. No
`canonical_post_state_hash`, successor-hash cache, hash exclusion rule, or
alternate canonical projection is permitted inside any canonical record or
ledger entry.
...
1. the exact `ExternalInputBoundaryPayload.v1.1` validator and fixed serializer/hash;
...
4. exact R0/Rinput/Rfinal/control records, pre-state-only ledger entries, external successor-hash witness, terminal dispositions, and tagged boundaries;
