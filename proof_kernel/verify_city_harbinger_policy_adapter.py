...
def load_policy(root: Path) -> tuple[dict[str, Any], Path]:
    ...
    required = {"schema", "city_frozen_contract", "harbinger_core", "required_unmapped_edge", "allowed_exact_test_controls", "allowed_exact_provenance_reads", "allowed_exact_build_execution", "retained_city_adversary_limit", "authority"}
    ...
    if (not isinstance(city, dict) or set(city) != {"path", "sha256"} or city["path"] != "proof_kernel/city_live_evidence_source_audit_contract.json"
            ...
            or not isinstance(value["allowed_exact_build_execution"], list)
...
def verify_frozen_sources(root: Path, policy: dict[str, Any]) -> None:
    ...
    for permitted in policy["allowed_exact_build_execution"]:
        if (not isinstance(permitted, dict) or set(permitted) != {"family", "path", "line", "callable", "token", "reference"}
                or permitted.get("family") != "function_to_consequence" or not isinstance(permitted.get("reference"), dict)
                or permitted["reference"] != {"path": permitted.get("path"), "function": permitted.get("callable"), "callee": permitted.get("token"), "line": permitted.get("line"), "consequence": "build_execution"}
                or permitted["reference"] not in exact_sites): reject()
...
def expected_decision(edge: dict[str, Any], policy: dict[str, Any]) -> dict[str, Any]:
    ...
    for permitted in policy["allowed_exact_build_execution"]:
        if not isinstance(permitted, dict): reject()
        if (family, path, line, callable_name, token) == (permitted.get("family"), permitted.get("path"), permitted.get("line"), permitted.get("callable"), permitted.get("token")):
            return {**base, "classification": "allowed", "city_classification": "build_execution", "city_reason_code": "lcer.exact_build_execution", "city_contract_reference": permitted.get("reference")}
