...
**Version:** 0.1.1  
**Status:** Frozen revised implementation specification. Not implemented.  
**Opened:** 2026-08-26  
...
The source record above remains immutable. The round-trip fixture normalizes its explicit access-point state to `intact` and records the parent hash. Its own canonical fixture hash is an implementation-evidence artifact, not a revision of the frozen counterfactual hash. On the accepted path only, that fixture hash is the transaction pre-state hash.
...
The transaction layer receives the proposal at one named decision boundary and creates the immutable authoritative pre-state `Rpre`. `hash(Rpre)` is the only current-record reference for the transaction.

### Validation

Validation evaluates every side-effect-free gate against the same `Rpre`. It does not short-circuit when a gate fails. Every gate result is recorded in canonical order, even when an earlier gate has already made the proposal ineligible to commit.

The required gates are:

1. Validate proposal schema and protocol/simulation compatibility.
2. Verify that `proposal.source_record_hash == hash(Rpre)`.
...
For the positive fixture, `hash(Rpre) == seed_fixture_hash`; this is a test condition, not a production rule. Every future physical proposal is compared to the authoritative record it claims to have observed.

### Commit

Commit is permitted only when every required validation gate passes. It atomically applies the three permitted mutations, then derives the route's unavailability from the new facts. Failed validation produces no mutation.
...
It must be rejected before mutation because its source record hash is stale relative to `Rpre` and its proposal id already has a terminal disposition. Validation still evaluates and records both failures. The canonical record hash must remain unchanged. The ledger must append an inspectable failed entry with:
...
| Duplicate/stale rejection | Replaying the accepted proposal changes no canonical fact, records both stale-source and duplicate-id gate failures, and adds one rejected ledger entry. |
...
This specification is frozen at `0.1.1`. Implement exactly this one consequence and prove its accepted, rejected, compressed, and freshly rematerialized paths. Do not add city scale or adjacent gameplay systems until all acceptance tests pass.

## Changelog

### 0.1.1 — 2026-08-26

- Replaced the fixture-specific stale-hash rule with record-relative optimistic concurrency: every proposal is checked against the transaction's immutable pre-state hash.
- Separated side-effect-free validation from commit. All validation gates must be evaluated and ledgered without short-circuiting; commit requires every required gate to pass.
- Preserved the reviewed original at [Bridge Access Persistence Round-Trip Proof — v0.1.0](Bridge%20Access%20Persistence%20Round-Trip%20Proof%20-%20v0.1.0.md).
