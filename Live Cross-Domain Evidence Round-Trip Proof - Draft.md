...
An operation wrapper binds schema, witness, domain, launch, operation and binding digest. Its content may be a committed projection or an emitted-Q reference. Process binding never grants a new consequence contract. Integrity validation and consequence validation are separate checks.
...
A projection contains only the exact committed arbitration record reference, domain, generation and resource owner needed to represent R0 or R1. Accept only the two pinned canonical identities. Reject provisional state, stale R0 after publication, wrong domain, altered binding and an owner inconsistent with canonical bytes.
...
Fields are enumerated in `observation_fields`. `anchor_count` and `resource_actor_count` are integers exactly one on success. `allocation_owner` is null at R0 and the string `domain_A` at R1. `all_proof_actor_ids` is a sorted, duplicate-free exhaustive list. Missing-world or incomplete-enumeration errors cannot become an empty successful census.

## Failure contract

The JSON contract enumerates F01–F18 with exact trigger, pre/post-commit boundary, normalized failure code, retained canonical record and affected representation. Retain the underlying predecessor rejection code as a separate diagnostic; normalized LCER codes never hide which validator rejected the operation.
...
The final release manifest is self-excluding, enumerates every new source/evidence member and references unchanged predecessor commitments. The JSON contract enumerates exactly 201 artifact files and 220 self-excluding release members. Each failed case still retains its raw process streams, record and trace. Missing captured inputs are explicit nulls in a failed record, never substituted physical evidence. These are candidate requirements; no release exists yet.

## Acquisition cost

Budget one successful UE build for each candidate source identity. One complete proposed acquisition has eight primary pair runs, eighteen failure pair runs and six canonical-fault pair runs: thirty-two pairs, sixty-four original launches and one replacement-process adversary. Offline verifier mutations add no UE processes. Each case has a 900-second outer limit and each blocking protocol operation a 60-second timeout. Actual wall time is unmeasured.
