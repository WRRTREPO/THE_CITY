...
**Corrective freeze authorized:** 2026-08-28
**Frozen proof-harness identity:** `SimultaneousPhysicalDomainsProof.v1.1` / `0.7.0-draft.77`
**Preserved predecessor contract:** [Simultaneous Physical Domains Proof v0.1.0](Simultaneous%20Physical%20Domains%20Proof%20-%20Draft.md)
...
proof_scope:
  ...
  release_manifest_members: 111 exact files_excluding_manifest
...
### Corrective proof-only W3 and fault-control channels

Version 0.1.0 froze no lawful Unreal-visible trigger for the exact W3 step and
no lawful selector for executing the refresh and physical-observation fault
matrices at their compiled boundaries. CPU sampling and Python replay therefore
cannot satisfy those witnesses. This v0.1.1 successor adds exactly two
proof-only command schemas on the already bound original stdin pipe. They are
not accepted by any primary, asymmetric, retention, destruction, or guard-open
positive-witness process.

After the harness has privately committed and verified H1, durably classified
both W3 domains stale, and opened the physical guard for H1, it sends exactly
one local-step command to each original W3 process:

```yaml
command_schema: SimultaneousPhysicalDomainLocalStepInvocation.v1
proof_scenario: simultaneous-physical-domains-v1.1
domain_role: domain_A | domain_B
...
The command contains no H1, current-head, guard, expected outcome, or canonical
capability. The adapter atomically samples its local counter, increments it by
exactly one, and emits:

```yaml
observation_schema: SimultaneousPhysicalDomainLocalStepObservation.v1
proof_scenario: simultaneous-physical-domains-v1.1
domain_role: exact bound role
operational_process_instance_id: exact original binding identity
process_binding_raw_sha256: exact original binding digest
step_id: stale_quarantine_step_0001
step_name: increment_nonconsequential_tick_counter_once
counter_before: integer_0_through_9007199254740990
counter_after: counter_before_plus_1
represented_hash_before: H0
represented_hash_after: H0
published_actor_identity_unchanged: true
materialization_receipt_count_delta: 0
canonical_evidence_count_delta: 0
canonical_scheduling_count_delta: 0
canonical_mutation_count_delta: 0
canonical_truth_claim_count_delta: 0
observation_source: live_ue_adapter_exact_local_step
```

The harness may record `proc_pidinfo` CPU samples around this command only as
supplemental process activity. A positive CPU delta without the exact step
observation fails W3.

Each refresh or physical-observation fault case uses a fresh isolated A/B
process pair. The target process has witness ID `f_refresh_fault` or
`f_physical_observation_fault`. After ordinary binding and at the exact command
slot declared below, the harness sends one fault-arm command:

```yaml
command_schema: SimultaneousPhysicalDomainFaultArmInvocation.v1
proof_scenario: simultaneous-physical-domains-v1.1
domain_role: domain_A
operation: arm_exact_fault_once
fault_run_id: exact surface/head/stage/edge identifier
fault_surface: refresh | physical_observation
fault_stage: exact member of the corresponding frozen stage list
fault_edge: before | after | at
target_head_role: H0 | H1
```

Cross-field law is exact: refresh requires H1 and `before|after`; physical
observation requires H0 or H1 and `at`. `fault_run_id` is respectively
`refresh/H1/STAGE/EDGE/domain_A` or
`physical_observation/HEAD/STAGE/at/domain_A`. Only domain A is injected; the
original domain B peer must remain alive and unchanged. The router validates
and stores this plan once, passes it only to the compiled adapter or probe, and
emits one `SimultaneousPhysicalDomainFaultArmReceipt.v1` binding the exact plan,
process instance, binding digest, and executable digest before the tested
operation is invoked.

At the selected compiled boundary the owning component records boundary entry,
records completion for an `after` edge or the exact instantaneous `at` edge,
and emits exactly one terminal
`SimultaneousPhysicalDomainInjectedFaultResult.v1`. The result binds the plan,
original process identity, executable digest, represented hash, local
publication state, receipt or observation outcome, and exact reason code. A
prepublication refresh fault leaves the domain stale. A fault after local
publication, at receipt emission, or during an H1 physical observation makes
it invalid. An H0 observation fault prevents initial synchronization. No fault
run may retry, refresh twice, or become a positive witness.

The `harness_receipt_observation_head_cross_check` fault is injected by the
harness after receipt of the live probe object; it remains bound to the same
fault-arm receipt and original process. No expected result, guard state, or head
observation enters Unreal.

These two schemas are the complete corrective addition. Fault selection through
argv, environment, bundle contents, filenames, directories, another descriptor,
debugger state, timing, shared memory, or witness-ID branching is prohibited.
...
proof_semantic_inputs:
  ...
  stdin_command_sequences:
    ...
    W3_stale_step_process:
      - exact bind_process_once invocation carrying the process binding
      - exact launch_physical_0001 inspection invocation
      - exact stale_quarantine_step_0001 local-step invocation
    refresh_fault_process:
      - exact bind_process_once invocation carrying the process binding
      - exact launch_physical_0001 inspection invocation
      - exact fault-arm invocation
      - exact h0_to_h1_refresh_0001 refresh invocation
    H0_observation_fault_process:
      - exact bind_process_once invocation carrying the process binding
      - exact fault-arm invocation
      - exact launch_physical_0001 inspection invocation
    H1_observation_fault_process:
      - exact bind_process_once invocation carrying the process binding
      - exact launch_physical_0001 inspection invocation
      - exact h0_to_h1_refresh_0001 refresh invocation
      - exact fault-arm invocation
  ...
  prohibited_hidden_semantic_inputs:
    ...
    - fault_selector_outside_exact_fault_arm_command
...
tuple, the refresh command, the W3 local-step command in W3, and the exact
fault-arm plan in a refresh-fault witness. The independent probe consumes only
the immutable binding, the two inspection commands, the exact fault-arm plan in
a physical-observation-fault witness, and the declared live Actor component
surfaces. Neither component may read a harness control-root path. Any value
outside the exact proof-semantic closure that reaches proof branch selection,
authoritative-derived construction, publication, receipt, observation
classification, or acceptance fails the witness.
...
stale domains receive their exact `stale_quarantine_step_0001` command and
execute the named `increment_nonconsequential_tick_counter_once` step. The
live adapter observation must prove the exact counter increased by one in each
original process while the represented hash and published Actor identity
remained unchanged. A process-wide CPU interval is supplemental only. The
local diagnostic may differ, but:
...
The case label is not evidence. The implementation must freeze and execute this
exact case-ID-to-action table:

```yaml
authority_case_actions:
  1: submit_H0_bytes_as_H1_receipt
  2: submit_H0_source_hash_in_H1_projection
  3: submit_H0_receipt_to_H1_head_disposition
  4: invoke_H0_scheduler_capability_against_H1
  5: invoke_H0_mutation_capability_against_H1
  6: submit_stale_disposition_as_synchronized
  7: submit_live_available_H0_observation_as_H1
  8: submit_local_route_override_to_canonical_payload_validation
  9: submit_local_competing_successor_to_canonical_payload_validation
  10: submit_domain_B_observation_under_domain_A_binding
  ...
  13: submit_projection_without_shared_route
  14: submit_projection_with_route_access_field
  15: submit_replacement_process_binding_for_H1_receipt
  16: submit_H1_receipt_after_injected_partial_publication
  17: execute_live_domain_destruction_and_refresh_failure_then_attempt_H1_change
  18: attempt_local_state_argument_on_canonical_resolver_and_measure
  19: execute_guard_open_exact_canonical_commit
  20: submit_bad_head_observation_to_guard_reopen
  21: execute_head_publication_failure_and_assert_failed_closed
  22: deliver_live_refresh_before_head_observation_and_reject_current_acceptance
  23: construct_head_observation_from_domain_source_and_reject
  24: submit_retained_scalar_to_authoritative_constructor
  25: execute_live_W5_poison_refresh_and_compare_H1
  26: submit_PID_reuse_binding_and_liveness_check
  27: send_live_alternate_and_second_refresh_commands
  28: stage_invalid_live_bundle_and_invoke_adapter
  29: send_head_observation_field_in_live_refresh_command
  30: attempt_guard_and_head_arguments_on_canonical_resolver
  31: submit_live_H1_receipt_without_probe
  32: submit_probe_observation_with_adapter_source
  33: execute_live_probe_invalid_surface_cases
  34: send_live_inspection_command_with_expected_outcome
  35: submit_synchronized_disposition_missing_prerequisites
  36: invoke_current_head_claim_from_stale_disposition
  37: send_live_undeclared_semantic_input_channels_and_audit
```

Every artifact row must include the exact action ID above, actual owning
function or live command path, concrete input, observed rejection stage and
reason, and any bound live witness/process identities. The verifier reconstructs
the table independently and rejects missing, duplicate, relabelled, reordered,
or swapped action IDs.

### Measured canonical before/after contract

Before and after every authority case and every refresh or physical-observation
fault case, the harness independently serializes the actual canonical record
and its history surfaces into this snapshot:

```yaml
snapshot_schema: SimultaneousPhysicalDomainsCanonicalMeasurement.v1
record_raw_sha256: SHA256_of_complete_stored_record_bytes
record_canonical_hash: SHA256_of_complete_canonical_record_bytes
authoritative_ledger_raw_sha256: SHA256_of_canonical_ledger_JSON
authoritative_ledger_entry_count: exact_integer
canonical_ancestry_raw_sha256: SHA256_of_canonical_ancestry_JSON_or_null
future_schedule_raw_sha256: SHA256_of_canonical_unresolved_work_JSON
future_schedule_entry_count: exact_integer
canonical_mutation_count: exact_ledger_entry_count
```

The artifact carries both complete snapshots and a recomputed relation. A
boolean such as `canonical_H1_unchanged` is accepted only as a derived summary
when every before/after field is present and equal. The verifier recomputes the
snapshots from the bound canonical bytes and rejects a hard-coded unchanged
claim, omitted history measurement, or mismatched digest.

The release verifier must execute in-memory negative mutations and prove
rejection of at least:

1. W3 with process-wide CPU evidence but no exact local-step observation;
2. any refresh or physical-observation fault row labelled live while lacking
   its original Unreal binding, executable identity, fault-arm receipt, and
   compiled-boundary result;
3. any swapped authority case label/action or case-ID/action mapping; and
4. any hard-coded unchanged summary whose measured canonical bytes or history
   surfaces differ, are omitted, or do not recompute.
...
All 36 cases execute in compiled Unreal code: one `before` and one `after`
injection for each listed stage. Each case uses a fresh original A/B process
pair, the exact H1 three-file bundle, the exact fault-arm receipt, and the exact
refresh command. Replaying live-produced JSON through the Python validator is
not execution of this surface. The released row must embed the target's complete
process binding, executable digest, fault-arm receipt, injected-fault result,
publication state, receipt outcome, disposition, peer liveness checkpoint, and
measured canonical before/after snapshots.
...
All 24 cases execute against compiled live probe/harness boundaries: each of
the twelve stages once at H0 and once at H1. Every case uses a fresh original
A/B pair and embeds the complete target process binding, executable digest,
fault-arm receipt, live injected-fault result or live observation for the
harness cross-check case, local publication state, observation outcome,
disposition, peer liveness checkpoint, and measured canonical before/after
snapshots. Python-only reconstruction is prohibited.
...
9. static call-graph/dataflow audit plus runtime input inventory proves the
   ...
   three visible bundle files, the W3 local-step command in W3, and the exact
   fault-arm plan in a refresh-fault process; it performs no head-observation
   check;
...
11. the command router's one-time binding verification is the only application
    ...
    probe have no environment read, undeclared command-line selector, cwd
    branch, project Content/ProofRecords read, alternate file-open, dynamic
    library lookup, or alternate descriptor read capable of affecting proof
    semantics; the exact fault-arm command is the sole declared proof selector;
...
18. the positive path's only refresh invocation is one exact line on each
    original stdin pipe; the W3 step and fault-arm schemas occur only in their
    declared witness command sequences; and each fixed role-specific
    visible-input directory has exactly three regular read-only members;
...
20. the independent physical probe reads only the immutable process binding,
    inspection command, exact fault-arm plan in a probe-fault process, and live
    UE world Actor/components, with no adapter record/candidate/receipt pointer,
    canonical/projection input, shared access variable, or expected-state input;
...
44 artifact names, and 111 unique self-excluding manifest members. It is
specification QA only: it does not import or execute a Phase-3 runtime, produce
proof evidence, or belong to the 44-member
artifact directory, 111-member release manifest, or release DAG. The authorized
...
commands, W3 step commands/results, fault-arm commands/receipts/results,
process-birth samples, pipe continuity, disposition receipts, canonical
before/after measurements, and source-audit facts must be embedded under their
exact witness or oracle member above. No additional log, screenshot, receipt,
cache dump, temporary file, or optional evidence member may be added to the
release directory. Runtime scratch output remains outside it and is not release
evidence.
...
L4A / L4B synchronized dispositions
        ↓
exact W3 local-step commands + live counter observations
        ↓
fresh bound fault-process pairs + exact fault-arm commands
        ↓
36 compiled refresh faults + 24 compiled probe/harness faults
        ↓
37 exact authority actions + measured canonical before/after snapshots
        ↓
...
the following exact relative paths: 111 manifest entries total, excluding the
manifest itself.
...
frozen_implementation_authority:
  ...
  evidence_path: Simultaneous Physical Domains Proof Evidence - v0.1.1.md
  artifact_directory: proof_kernel/SimultaneousPhysicalDomainsProofRecords/
  artifact_member_count: 44
  manifest_path: Simultaneous Physical Domains Proof - v0.1.1 SHA256SUMS.txt
  manifest_member_count_excluding_manifest: 111
...
It must exclude itself. Its 111 member lines must be the complete union above,
...
freeze_acceptance:
  ...
  W3_local_step_command_and_observation: exact_live_UE_counter_step
  fault_arm_command: exact_fault_witness_only_original_stdin_pipe
  live_refresh_fault_execution: exact_18_stages_before_and_after
  live_physical_observation_fault_execution: exact_12_stages_at_H0_and_H1
  authority_case_action_table: exact_37_rows
  canonical_before_after_measurement: exact_record_ledger_ancestry_schedule
  ...
  release_manifest: exact_111_self_excluding_and_mechanically_verified
...
contract retains v0.1.0's physical guard, claim, live-UE oracle, and positive
input closure while adding only the declared W3/fault command branches. It
freezes the exact 44/111 release boundary without claiming runtime evidence.
...
- Preserved the frozen v0.1.0 contract and pushed implementation history, then
  opened this versioned corrective successor after independent evidence review
  proved that CPU-only W3 evidence and Python-replayed fault matrices were not
  admissible runtime proof.
- Added one exact W3 local-step command/observation and one exact fault-arm
  command/receipt/result channel on the original bound stdin/stdout pipes.
  Positive witnesses reject both additions, and every alternate selector
  remains prohibited.
- Froze 36 compiled refresh fault cases, 24 compiled live-probe/harness fault
  cases, the exact 37-row case-ID/action table, and actual canonical
  record/ledger/ancestry/schedule before/after measurements.
- Added the required verifier-negative obligations for CPU-only W3,
  Python-only live matrices, swapped authority actions, and hard-coded
  unchanged claims.
- Preserved 44 artifact roles and expanded the self-excluding release to 111
  members so both the v0.1.0 predecessor contract and this v0.1.1 successor are
  bound. Evidence remains unsealed, capacity remains v0.1.11, and Phase 4
  remains closed.
