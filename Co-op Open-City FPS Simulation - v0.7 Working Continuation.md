...
- Implemented only the exact frozen Phase-4 surface: four Python paths, ten
  Unreal paths, the bounded `CityProofGameMode.cpp` branch, candidate evidence,
  82 proof artifacts, the release verifier, and the authorized governing
  updates.
...
- Passed all 6 controls, 4 asymmetric failures, 23 binding adversaries, 121
  authority subcases, 18 head faults, 138 materialization faults, 36
  observation faults, and 6 liveness adversaries; the source audit passes
...
- Generated the exact 82-member artifact directory. Its independent verifier
  ...
  regenerates all artifacts in isolation, and rejects 34/34 in-memory
...
- Independent exact-object review rejected first corrective candidate
  `9944799d3f704dce82c204387fbd1b37aa19b2bd`: a global rewrite of all 441
  loaded-image embeddings, all 432 registry digests, the proof-run relations,
  and the manifest could still substitute a common four-row census and fake
  executable/module UUIDs. The successor binds every occurrence to the exact
  acquired 1,719-row census commitment and adds that full coordinated rewrite
  as a non-vacuous verifier adversary.
